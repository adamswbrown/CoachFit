...
const settingsSchema = z.object({
  ...
  classBookingEnabled: z.boolean().optional(),
  bookingTimezone: z.string().min(1).max(80).optional(),
  bookingCurrency: z.string().min(3).max(3).optional(),
  defaultClassCapacity: z.number().int().min(1).max(200).optional(),
  defaultWaitlistCap: z.number().int().min(0).max(200).optional(),
  bookingOpenHoursDefault: z.number().int().min(0).max(24 * 60).optional(),
  bookingCloseMinutesDefault: z.number().int().min(0).max(24 * 60).optional(),
  lateCancelCutoffMinutesDefault: z.number().int().min(0).max(24 * 60).optional(),
  defaultCreditsPerBooking: z.number().int().min(0).max(20).optional(),
      ...
      if (!settings) {
        settings = await db.systemSettings.create({
          data: {
            ...
            classBookingEnabled: normalizedData.classBookingEnabled ?? DEFAULT_SETTINGS.classBookingEnabled,
            bookingTimezone: normalizedData.bookingTimezone ?? DEFAULT_SETTINGS.bookingTimezone,
            bookingCurrency: normalizedData.bookingCurrency ?? DEFAULT_SETTINGS.bookingCurrency,
            defaultClassCapacity: normalizedData.defaultClassCapacity ?? DEFAULT_SETTINGS.defaultClassCapacity,
            defaultWaitlistCap: normalizedData.defaultWaitlistCap ?? DEFAULT_SETTINGS.defaultWaitlistCap,
            bookingOpenHoursDefault:
              normalizedData.bookingOpenHoursDefault ?? DEFAULT_SETTINGS.bookingOpenHoursDefault,
            bookingCloseMinutesDefault:
              normalizedData.bookingCloseMinutesDefault ?? DEFAULT_SETTINGS.bookingCloseMinutesDefault,
            lateCancelCutoffMinutesDefault:
              normalizedData.lateCancelCutoffMinutesDefault ??
              DEFAULT_SETTINGS.lateCancelCutoffMinutesDefault,
            defaultCreditsPerBooking:
              normalizedData.defaultCreditsPerBooking ?? DEFAULT_SETTINGS.defaultCreditsPerBooking,
