...
- **Class booking**: Schedule view, tap-to-book with credit debit, cancellation with refund, 14-day booking window
- **Credit system**: Credit products (packs, subscriptions), balance/ledger, manual adjustments, submission approval
- **Challenge lifecycle**: Enrollment, progress tracking (streak, check-in rate), coach compliance dashboard
...
npm run db:seed          # Seed initial data (test users + class templates/sessions)
...
18. **ClassTemplate**: Reusable class definition
    - `ownerCoachId`, `name`, `classType` (HIIT, CORE, Strength), `capacity`, `locationLabel`
    - Booking rules: `bookingOpenHoursBefore` (default 336 = 14 days), `cancelCutoffMinutes` (default 120 = 2 hours), `creditsRequired` (default 1)
    - `waitlistEnabled`, `waitlistCapacity`, `scope` (FACILITY or COHORT_ONLY)
    - Has many ClassSessions

19. **ClassSession**: Individual class occurrence
    - `classTemplateId`, `instructorId`, `startsAt`, `endsAt`, `capacityOverride`
    - `status`: SCHEDULED, CANCELLED, COMPLETED
    - Has many ClassBookings

20. **ClassBooking**: Client booking for a session
    - `sessionId`, `clientId`, `status`: BOOKED, WAITLISTED, CANCELLED, LATE_CANCEL, ATTENDED, NO_SHOW
    - `source`: CLIENT, COACH, ADMIN, WAITLIST_AUTO
    - `@@unique([sessionId, clientId])` — one booking per client per session
    - Links to `creditLedgerEntries` for credit tracking

21. **CreditProduct**: Purchasable credit pack or subscription
    - Modes: MONTHLY_TOPUP, ONE_TIME_PACK, CATALOG_ONLY
    - `purchasePriceGbp`, `creditsPerPeriod`, `periodType`, `allowRepeatPurchase`, `rolloverPolicy`
    - `purchasableByProviderOnly` for coach-only products

22. **ClientCreditAccount**: Per-client credit balance (denormalized)
    - `@@unique([clientId])` — one account per client
    - `balance` updated atomically via `updateMany` with `balance >= amount` guard

23. **ClientCreditLedger**: Append-only credit transaction journal
    - `deltaCredits` (+/-), `reason` (TOPUP_MONTHLY, PACK_PURCHASE, BOOKING_DEBIT, REFUND, MANUAL_ADJUSTMENT, EXPIRY)
    - Links to booking, submission, product, subscription for provenance
...
│   ├── classes/            # Class schedule + booking (14-day date strip, tap-to-book)
│   │   └── bookings/      # Upcoming and past bookings list
│   ├── challenges/         # Available challenges + enrollment
│   │   └── [cohortId]/    # Challenge progress (ring, streak calendar, stats)
│   └── credits/           # Credit balance, purchase, transaction history
├── coach-dashboard/        # Coach views (cohort management, analytics, weekly review, healthkit data, questionnaire analytics)
│   ├── classes/            # Schedule management (week view, session CRUD)
│   ├── challenges/         # Challenge cohort management, participant compliance grid
│   └── credits/           # Client credit overview, adjustments, submission queue
...
│   ├── classes/          # Class scheduling & booking APIs (schedule, book, bookings, sessions, templates)
│   ├── challenges/       # Challenge lifecycle APIs (list, enroll, progress, active, history)
│   ├── credits/          # Credit system APIs (products, balance, ledger, submit, adjust, subscribe)
...
├── booking.ts            # Class booking logic (createBooking, cancelBooking, getAvailableSlots, getClientBookings)
├── challenges.ts         # Challenge lifecycle (enrollInChallenge, getChallengeProgress, getActiveChallenges, getChallengeHistory)
├── class-schedule.ts     # Schedule management (createClassSession, bulkCreateSessions, getSchedule)
├── credits.ts            # Credit operations (debitCredits, refundCredits, creditTopup, getBalance, adjustCredits, processSubmission)
├── db.ts                 # Prisma client instance
├── email.ts              # Email service (Resend)
├── permissions.ts        # Role-based permissions
├── audit-log.ts          # logAuditAction() for all mutations
├── security/
...
└── validations/
    ├── healthkit.ts      # HealthKit/ingest Zod schemas
    ├── booking.ts        # Class booking Zod schemas
    ├── challenges.ts     # Challenge Zod schemas
    └── credits.ts        # Credit system Zod schemas
...
├── seed.ts               # Seed script for test users (calls seed-classes.ts)
└── seed-classes.ts       # Seed ClassTemplates + ClassSessions from TeamUp data (idempotent, 6-week window)
...
- Challenge history uses audit log (CHALLENGE_ENROLL events) since membership is single-slot

**Class Booking System** (lib/booking.ts):
- All booking mutations use `db.$transaction()` with inline credit operations (not nested transactions)
- `createBooking()`: validates capacity, booking window (14 days ahead), deduplication, then debits credits atomically
- `cancelBooking()`: before cutoff → CANCELLED + credit refund; after cutoff → LATE_CANCEL, no refund
- Credit debit uses atomic `updateMany` with `balance >= amount` guard to prevent race conditions
- `@@unique([sessionId, clientId])` prevents double-booking
- Booking window: `bookingOpenHoursBefore` (default 336h = 14 days) and `bookingCloseMinutesBefore` (default 0)
- Cancel cutoff: `cancelCutoffMinutes` (default 120 = 2 hours before class)

**Challenge Lifecycle** (lib/challenges.ts):
- `enrollInChallenge()`: checks user not already in any cohort (single-cohort model), validates CHALLENGE type
- `getChallengeProgress()`: calculates from Entry records within challenge date window (works after membership removal)
- `getChallengeHistory()`: queries AdminAction audit log for CHALLENGE_ENROLL events (preserves history after membership archival)
- Challenge progress: daysCompleted, streak (consecutive days backwards), weeklyEntries, checkInRate, percentComplete

**Credit System** (lib/credits.ts):
- Ledger-based: all operations create ClientCreditLedger entries (append-only)
- `debitCredits()`: atomic updateMany with `balance >= amount` prevents overdraft race conditions
- `refundCredits()`: convenience wrapper with REFUND reason, linked to bookingId
- `processSubmission()`: approve/reject credit purchase submissions (idempotent — skips already-processed)

**Seeded Class Data**:
- 3 templates: HIIT (cap 10), CORE (cap 15), Strength (cap 12)
- ~270 HIIT/CORE sessions from real TeamUp data (6-week window: 2026-03-21 → 2026-05-01)
- ~18 Strength sessions (Mon 18:00, Wed 18:00, Sat 11:15, round-robin instructors)
- Idempotent seed: `npm run seed:classes` — skips existing sessions, safe to re-run
...
**Fetching User's Cohort (Client)** (single-cohort model):
```typescript
